...
async function createWidget() {
    ...
    if (useIcloud) {
        ...
        for (let contact of contactsInIos) {
            ...
            contactsName = contactsName.split(' ')[0] + " " + contact.familyName;
...
function createTable (){
   ...
   for (let i of allContainers){
		...
		if (yes_no == "yes") { yes_no_text = "✅"} else { yes_no_text = "⭕️"}
	   var ticks = row.addButton(yes_no_text);
...
function addContactInfoToRow(customContact, row) {
    ...
    daysInfoText.textColor = Color.red();
